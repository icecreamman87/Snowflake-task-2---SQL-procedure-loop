...
AS $$
BEGIN
    RETURN 'call test_2(''' || :param1 || ''',''' || :param2 || ''',''' || :param3 || ''');';
END;
$$;
CREATE OR REPLACE PROCEDURE run_test_2_cursor()
RETURNS ARRAY
LANGUAGE SQL
AS $$
DECLARE
    my_cursor CURSOR FOR SELECT date1, ts, date2 FROM test_2_params;
    call_result VARCHAR;
...
BEGIN
    FOR row_data IN my_cursor DO
        ...
        CALL test_2(:v_date1, :v_ts, :v_date2) INTO :call_result;
        log_array := ARRAY_APPEND(:log_array, :call_result);
    END FOR;    
    RETURN :log_array;
END;
$$;
CALL run_test_2_cursor();
--Version_2:
CREATE OR REPLACE PROCEDURE run_test_2_resultset()
RETURNS ARRAY
LANGUAGE SQL
AS $$
DECLARE
    my_rs RESULTSET DEFAULT (SELECT date1, ts, date2 FROM test_2_params);
    call_result VARCHAR;
    log_array ARRAY DEFAULT [];
    v_date1 VARCHAR;
    v_ts VARCHAR;
